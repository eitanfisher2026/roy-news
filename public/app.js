  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 24, background: C.bg }}>
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        ...
        <h1 style={{ fontSize: 30, fontWeight: 800, margin: '0 0 8px', color: C.text }}>Airtime</h1>
        <div style={{ color: C.muted, fontSize: 14, maxWidth: 320 }}>
          Your daily media briefing — curated, translated, and delivered on schedule
        </div>
      ...
      <div className="panel" style={{ padding: 28, maxWidth: 340, width: '100%', textAlign: 'center' }}>
        <div style={{ color: C.muted, fontSize: 13, marginBottom: 20 }}>
          Sign in to manage your scheduled reports
        </div>
        <button
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 24, background: C.bg, textAlign: 'center' }}>
      ...
      <div style={{ color: C.muted, fontSize: 14, maxWidth: 340, marginBottom: 4 }}>
        {user.email} isn't authorized to use Airtime yet.
      </div>
...
// ─── Setup Config Panel (shared by SourceManager's new/refresh flows) ─────────
...
function SettingsPage({ onBack, deferredInstall, user, onSignOut, isAdmin }) {
  ...
  // Topics — the shared registry backing every schedule. useTopicRegistry
  // live-subscribes, so this section never shows stale data even if another
  // tab edits a topic while it's open.
    ...
    <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 20px' }}>
      <div className="panel" style={{ padding: 24 }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 11, color: C.faint, marginTop: 1 }}>Add or remove people who can use Airtime</div>
                </div>
              </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 12, color: C.faint, marginBottom: 14, lineHeight: 1.6 }}>
            Airtime runs on your own API key — pick a provider and paste your key below.
          </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 12 }}>Install Airtime on your phone or desktop for quick access — works like a native app.</p>
...
function RawScheduledRunView({ scheduleCountry, dateLabel, run, onDelete, onClose, user, scheduleId, runId, onSummaryGenerated }) {
  ...
  async function handleShareResult() {
    const text = buildRawShareText({ name: scheduleCountry }, dateLabel, displayRun);
    const title = `Airtime — ${scheduleCountry} · ${formatDateLabelDMY(dateLabel)}`;
    if (navigator.share) {
  ...
  return (
    <div className="panel print-root" style={{ maxWidth: 640, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: 20 }} onClick={e => e.stopPropagation()}>
      <div className="print-header" style={{ display: 'none' }}>
        <div style={{ fontSize: 22, fontWeight: 800 }}>📰 Airtime — {scheduleCountry}</div>
...
function ScheduledReportsPanel({ user, countries, defaultOpen = false }) {
  ...
  // Sharing — owner-only, shown inside the edit dialog. "read" can view the
  // schedule and its reports; "write" can also edit/pause/delete it, same
  // two-level model as Buli lists.
    ...
    confirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setEditingId(prev => prev === schedule.id ? null : prev);
        } catch (e) {
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        <span title={s.enabled ? 'Enabled' : 'Disabled'} style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: s.enabled ? '#3b82f6' : '#ef4444', flexShrink: 0 }} />
            ...
            <div className="panel" style={{ maxWidth: 560, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: 20 }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {s.access !== 'read' && (
                    <button onClick={() => toggleEnabled(s)} disabled={busyId === s.id}
                      style={{ ...SMALL_BTN, color: s.enabled ? '#4ade80' : C.faint, borderColor: s.enabled ? '#14532d' : C.border, fontSize: 11, padding: '5px 10px' }}>
                      {s.enabled ? '● On' : '○ Off'}
                    </button>
                  )}
              ...
              {s.access !== 'read' && (
                <div style={{ marginTop: 14 }}>
                  <label style={{ display: 'block', fontSize: 11, color: C.faint, marginBottom: 4 }}>Send now</label>
              ...
              {s.access === 'owner' && (
                <div style={{ marginTop: 14, padding: 12, background: '#0a1626', borderRadius: 7, border: '1px solid ' + C.border }}>
...
function App() {
  ...
  const [page, setPage] = useState('main'); // 'main' = Scheduled Reports (the only content page now), 'settings' = Settings
  ...
  // cached for the rest of the session — needs the full source list per country.
  useEffect(() => {
    if (page !== 'main' || scheduleCountriesLoaded) return;
    db.ref('countries').once('value').then(snap => {
      const val = snap.val();
  ...
  // Mobile back button — navigate within app instead of exiting. Only real
  // navigation left is Settings <-> the (single) Scheduled Reports home.
  useEffect(() => {
    window.history.pushState({ airtime: true }, '');
    function onPop() {
      if (page === 'settings') { setPage('main'); window.history.pushState({ airtime: true }, ''); }
    }
  ...
  const header = (
    <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 20px', background: '#0a1526', borderBottom: '1px solid ' + C.border, position: 'sticky', top: 0, zIndex: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontWeight: 800, fontSize: 16, color: C.text }}>Airtime</span>
        <span className="version-tag" style={{ cursor: 'pointer' }} onClick={() => window.location.reload()} title="Tap to refresh">{VERSION}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {/* Share app */}
        <button
          onClick={() => navigator.share ? navigator.share({ title: 'Airtime', url: 'https://roy-news-23ab4.web.app' }).catch(()=>{}) : navigator.clipboard?.writeText('https://roy-news-23ab4.web.app').then(() => alert('Link copied!'))}
          style={SMALL_BTN}
          title="Share Airtime"
        >
          🔗 Share
  ...
  function handleSignOut() { auth.signOut(); setPage('main'); }

  // 'main' is the only content page besides Settings now — Scheduled
  // Reports is the whole app, not one destination among several.
  const content = page === 'settings'
    ? <SettingsPage onBack={() => setPage('main')} deferredInstall={deferredInstall} user={user} onSignOut={handleSignOut} isAdmin={role === 'admin'} />
    : (
