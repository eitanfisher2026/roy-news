...
function extractRawTexts(days) {
  const texts = [], paths = [];
  (days || []).forEach((d, di) => {
    (d.topics || []).forEach((t, ti) => {
      (t.sources || []).forEach((s, si) => {
        (s.articles || []).forEach((a, ai) => {
          if (a.title) { texts.push(a.title); paths.push([di, 'topics', ti, 'sources', si, 'articles', ai, 'title']); }
          if (a.text) { texts.push(a.text); paths.push([di, 'topics', ti, 'sources', si, 'articles', ai, 'text']); }
        });
      });
    });
  });
...
function applyRawTranslations(days, paths, translations) {
...
function buildRawShareText(country, dateLabel, days) {
...
function RawScheduledRunView({ scheduleCountry, dateLabel, run, onDelete, onClose }) {
  const [isHebrew, setIsHebrew] = useState(false);
  const [hebrewDays, setHebrewDays] = useState(null);
  const [translating, setTranslating] = useState(false);
  const [translateProgress, setTranslateProgress] = useState(0);
  const [shareCopied, setShareCopied] = useState(false);
  const [resultCopied, setResultCopied] = useState(false);
  ...
  async function handleTranslate() {
    if (isHebrew) { setIsHebrew(false); return; }
    if (hebrewDays) { setIsHebrew(true); return; }
    setTranslating(true);
    setTranslateProgress(0);
    try {
      const { texts, paths } = extractRawTexts(run.days);
      const CONCURRENCY = 6;
      const allTranslations = new Array(texts.length);
      for (let i = 0; i < texts.length; i += CONCURRENCY) {
        const batch = texts.slice(i, i + CONCURRENCY);
      ...
      setHebrewDays(applyRawTranslations(run.days, paths, allTranslations));
      setIsHebrew(true);
    } catch (e) {
  ...
  async function handleCopyResult() {
    const text = buildRawShareText({ name: scheduleCountry }, dateLabel, displayDays);
    await navigator.clipboard?.writeText(text);
    setResultCopied(true);
    setTimeout(() => setResultCopied(false), 2500);
  }
    ...
    <div className="panel" style={{ maxWidth: 640, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: 20 }} onClick={e => e.stopPropagation()}>
      ...
      {displayDays.length === 0 ? (
        ...
        <div key={di} style={{ marginBottom: 20 }}>
          ...
          {(d.topics || []).map((t, ti) => (
            <div key={ti} style={{ marginBottom: 14, paddingLeft: 10, borderLeft: '2px solid ' + C.border }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 8 }}>Topic: {t.label}</div>
              {(t.sources || []).map((s, si) => (
