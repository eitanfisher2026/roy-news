  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 24, background: C.bg }}>
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        ...
        <h1 style={{ fontSize: 30, fontWeight: 800, margin: '0 0 8px', color: C.text }}>PressWatch</h1>
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 24, background: C.bg, textAlign: 'center' }}>
      ...
      <div style={{ color: C.muted, fontSize: 14, maxWidth: 340, marginBottom: 4 }}>
        {user.email} isn't authorized to use PressWatch yet.
      </div>
...
function buildShareText(country, date, results, includeIsrael) {
  let text = `📰 PressWatch — ${country.name}\n${formatDisplayDate(date)}\n`;
    ...
    <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 20px' }}>
      <div className="panel" style={{ padding: 24 }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 11, color: C.faint, marginTop: 1 }}>Add or remove people who can use PressWatch</div>
                </div>
              </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 12 }}>Install PressWatch on your phone or desktop for quick access — works like a native app.</p>
        ...
        <div style={{ textAlign: 'center', fontSize: 11, color: C.faint, marginTop: 24, paddingTop: 16, borderTop: '1px solid ' + C.border }}>
          © {new Date().getFullYear()} PressWatch
        </div>
      </div>
      {confirmDialog}
      {suggestModal && (
...
function RawScheduledRunView({ scheduleCountry, dateLabel, run, onDelete, onClose, user, scheduleId, runId, onSummaryGenerated }) {
  ...
  async function handleShareResult() {
    const text = buildRawShareText({ name: scheduleCountry }, dateLabel, displayRun);
    const title = `PressWatch — ${scheduleCountry} · ${formatDateLabelDMY(dateLabel)}`;
    if (navigator.share) {
  ...
  return (
    <div className="panel print-root" style={{ maxWidth: 640, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: 20 }} onClick={e => e.stopPropagation()}>
      <div className="print-header" style={{ display: 'none' }}>
        <div style={{ fontSize: 22, fontWeight: 800 }}>📰 PressWatch — {scheduleCountry}</div>
...
function App() {
  ...
  useEffect(() => {
    window.history.pushState({ presswatch: true }, '');
    function onPop() {
      if (page === 'settings') { setPage('main'); window.history.pushState({ presswatch: true }, ''); }
    }
  ...
  const header = (
    <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 20px', background: '#0a1526', borderBottom: '1px solid ' + C.border, position: 'sticky', top: 0, zIndex: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontWeight: 800, fontSize: 16, color: C.text }}>PressWatch</span>
        <span className="version-tag" style={{ cursor: 'pointer' }} onClick={() => window.location.reload()} title="Tap to refresh">{VERSION}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {/* Share app */}
        <button
          onClick={() => navigator.share ? navigator.share({ title: 'PressWatch', url: 'https://roy-news-23ab4.web.app' }).catch(()=>{}) : navigator.clipboard?.writeText('https://roy-news-23ab4.web.app').then(() => alert('Link copied!'))}
          style={SMALL_BTN}
          title="Share PressWatch"
        >
          🔗 Share
        </button>
        <button
